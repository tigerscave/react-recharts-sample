...
class IntractPieChartContainer extends React.Component {
  constructor(props){
    ...
    this.onPiaEnter = (data) => {

      console.log('this is enter data ...', data);
      this.setState({
    ...
    this.onSelectedClick = (data) => {
      console.log('...optiion clicked');
  ...
  render() {
    console.warn(latestLargeCategories);
    ...
    return (
      <div>
        ...
        <SalesPieChart
          ...
          onPiaEnter={this.onPiaEnter}
          onPiaLeave={this.onPiaLeave}
          data={latestLargeCategories} />
        <div>
          <select onChange={this.onSelectedClick}>
            <option>None</option>
            {latestLargeCategories.map(largeCategory => (
              <option>{largeCategory.categoryCode}</option>
            ))}
          </select>
        </div>
      </div>
